...
SPACE = {'n_filters':      hp.choice('n_filters',      PRMTRS['n_filters']),
         ...
         'kernel_size':    hp.choice('kernel_size',    PRMTRS['kernel_size']),
...
def objective(params):
    ...
    # Data generators
    batch_size = 10  # change according to your GPU limitations
    train_generator = PairGenerator(X_train, y_train, seed=1, 
                                  amount_of_pairs=0.25, #1.0
    ...
    try: # If che model istance doen't give an error, it start the training and the test
...
if __name__ == '__main__':   
    ...
    X, y = pre_processing.get_data(ESC50_PATH, FULL_DATA=True, feature='mel')    
    
    # Split data into training, oneshot training and oneshot validation sets
    X_train, X, y_train, y = pre_processing.class_split2(X, y, split_size=0.6)
    X_oneshot_val, X_oneshot_train, y_oneshot_val, y_oneshot_train = pre_processing.class_split2(X, y, split_size=0.5)

    
    X,y = None, None  # release memory
    X_2,y_2 = None, None  # release memory
