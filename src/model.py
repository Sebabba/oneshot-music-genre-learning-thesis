...
def create_siamese_model(input_shape, 
                       n_filters=64, n_dense=125,
                       n_conv_layers=3, n_dense_layers=1, 
                       dist='l2', 
                       dropout_dense=0.25, dropout_conv=0.25,
                       learning_rate=0.0038, decay=7.048e-07,
                       loss='mse', kernel_size=4):
    ...
    for i in range(n_conv_layers):
        cnn.add(Conv2D(n_filters, (kernel_size, kernel_size),
                       padding='valid'))
        cnn.add(BatchNormalization())
...
def test_oneshot(mdl, X_oneshot, y_oneshot, visualize=True, save_results=True, seed=None, n_samples_per_classes=20):
    ...
    n_tests = int(n_samples_per_classes/2)
    ...
    shuffle_ids = np.arange(n_samples_per_classes)
